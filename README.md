...
- **실행 플래그**:  
\- RUN_ONLY_STAGE_2: True로 설정하면 1단계(모델 학습)를 건너뛰고, 스크립트는 이전 실행 결과를 로드하여 2단계(클러스터링 및 시각화)를 실행하려고 시도합니다.  
\- PERFORMANCE_FILE: 1단계 결과를 저장/로드할 파일의 이름입니다.  

**전역 매개변수**:  
- clean_col_names(): 특수 문자를 밑줄(_)로 바꿔 컬럼 이름을 정제하는 함수입니다.  
- encodings, scalers: 반복적으로 적용할 인코딩 및 스케일링 방법들의 목록입니다.  
- N_ESTIMATORS, RANDOM_STATE, N_JOBS, N_SPLITS_KFOLD: 모델 및 교차 검증을 위한 공통 하이퍼파라미터입니다. (N_ESTIMATORS: 추정기 개수, RANDOM_STATE: 난수 시드, N_JOBS: 사용할 CPU 코어 수, N_SPLITS_KFOLD: K-겹 교차 검증 분할 수)  
- PLOTS_DIR: 생성된 시각화 자료를 저장할 디렉토리입니다.  
 
#### 1단계: 모델 학습, (계층적 K-겹 교차 검증을 사용한) 평가 및 예측 생성  
이 단계는 지정된 인코딩 및 스케일링 방법의 모든 조합을 반복합니다.  
**1. 데이터 로딩 및 전처리 (각 조합별)**:  
- 전처리된 학습 및 테스트 세트의 파일 이름을 구성합니다 (예: Label_Train_Slim_MinMax.csv).  
- 특정 조합에 대한 데이터가 없을 경우 FileNotFoundError를 처리합니다.  
- 특성(X_train_full, X_test_final)과 타겟(y_train_full)을 분리합니다.  
- clean_col_names()를 사용하여 컬럼 이름을 정제합니다.  
- 테스트 세트(X_test_final)가 학습 세트와 동일한 컬럼을 갖도록 보장하고, 누락된 컬럼은 0으로 채웁니다.  

**2. 계층적 K-fold 교차 검증 (각 조합별)**:  
- StratifiedKFold를 사용하여 교차 검증의 각 폴드(fold)가 전체 데이터셋과 유사한 타겟 클래스 비율을 갖도록 합니다. 이는 불균형 데이터셋에 매우 중요합니다.  
- 각 폴드에 대해:   
...
**3.교차 검증 결과 집계 (각 조합별)**:  
- 각 모델에 대해 모든 폴드에 걸쳐 AUC, F1 및 RMSE의 평균과 표준 편차를 계산합니다.  
- 이러한 평균 CV 결과를 출력합니다.  
- 현재 인코딩/스케일링 조합에 대해 XGBoost, LightGBM 또는 랜덤 포레스트 Classifier가 달성한 최대 평균 Classifier AUC(max_classifier_avg_cv_auc)를 식별합니다.    

**4.조합 성능 저장 (각 조합별)**:  
- all_combination_performance 목록에 딕셔너리를 추가합니다. 이 딕셔너리에는 다음이 포함됩니다:  
\- 인코딩 및 스케일링 방법.  
...
- 교차 검증 후, 5개 모델 각각은 이번에는 전체 X_train_full 및 y_train_full 데이터로 다시 학습됩니다.  
- XGBoost/LightGBM Classifier를 위해 전체 학습 데이터를 사용하여 full_scale_pos_weight가 계산됩니다.  
- 이러한 전체 데이터 모델은 X_test_final 데이터에 대한 예측을 생성하는 데 사용됩니다.  
- Classifier 예측(확률) 및 Regressor 예측(연속 값)은 CSV 파일(예: Label_MinMax_XGB_Clf_Probs_predictions.csv)로 저장됩니다.  

**6.1단계 결과 저장**:  
- all_combination_performance가 비어있지 않으면 피클 파일(PERFORMANCE_FILE)로 저장됩니다.  
- can_proceed_to_stage_2 플래그가 True로 설정됩니다.  
 
________________________________________
**RUN_ONLY_STAGE_2 = True 처리**  
- 이 플래그가 설정되면 1단계가 건너뛰어집니다.  
- 스크립트는 PERFORMANCE_FILE에서 all_combination_performance를 로드하려고 시도합니다.  
- FileNotFoundError 또는 로드된 파일이 비어있는 경우에 대한 오류 처리가 구현되어 있습니다.  
- 성공적인 로딩 여부에 따라 can_proceed_to_stage_2가 설정됩니다.  


**2단계: 상위 5개 조합에 대한 K-평균 클러스터링 및 시각화**  
이 단계는 can_proceed_to_stage_2가 참이고 all_combination_performance가 채워져 있는 경우에만 실행됩니다.  
**1. 상위 조합 선택**:  
- all_combination_performance는 max_classifier_val_auc (실제로는 1단계의 최대 평균 CV AUC임)를 기준으로 내림차순으로 정렬됩니다.  
- 상위 5개 조합이 선택됩니다.  
 
**2.각 상위 조합 처리**:  
상위 5개 조합 각각에 대해:  
- 시각화를 위해 PLOTS_DIR 아래에 전용 하위 디렉토리가 생성됩니다 (예: model_visualizations/TOP5_CV_Label_MinMax).  
- **클러스터링을 위한 데이터 로딩**:  
...
### Output:  
- **1단계**: 
\- 각 모델 및 데이터 조합에 대한 학습 과정 및 CV 결과를 자세히 보여주는 콘솔 출력.  
\- 각 모델 및 데이터 조합에 대한 테스트 세트 예측  
\- 하나의 인코딩-스케일링 조합에 대한 성능 요약 및 예측 파일 경로를 담은 딕셔너리 목록을 저장하는 피클 파일(all_combination_performance_cv.pkl).  
- **2단계**:  
